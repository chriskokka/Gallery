...
function stripURIForEmail($URI){
	return str_replace('/zenphoto/index.php?p=order&Uri=/zenphoto/cache/','album=',$URI);
}

function stripURIForGallery($URI){
	$tempString = str_replace('http://127.0.0.1:4001/zenphoto/index.php?album=','',$URI);
	$position = strpos($tempString,'&image=');
	$length = strlen($tempString);
	$difference = ($position + 7)- $length; /**  NOTE: RESULT IS ALREADY !!NEGATIVE!!  */
	$tempString2 = substr($tempString,$difference);
	$arr = explode(".jpg", $tempString2, 2);
	$result = $arr[0];
	return $result;
}
